...
func TestKeepAliveListenKeyService_validate(t *testing.T) {
	t.Run("valid config", func(t *testing.T) {
		svc := NewKeepAliveListenKeyService("", "").
			ListenKey("fake").
			RecvWindow(50)
...
func TestKeepAliveListenKeyService_buildQuery(t *testing.T) {
	mockTime := func() int64 { return 1620000000000 }
	secretKey := "my-secret-key"

	recvWindow := int64(5000)

	svc := NewKeepAliveListenKeyService("api-key", secretKey).
		ListenKey("fake").
...
func TestNewKeepAliveListenKeyService_Validate_ErrorsWrapped(t *testing.T) {
	svc := NewKeepAliveListenKeyService("", "")
	...
	assert.Equal(t, "KeepAliveListenKeyService.Validate", sdkErr.Op())
	assert.Contains(t, sdkErr.Message(), "listenKey is required")
}

func TestKeepAliveListenKeyService_Do_Success(t *testing.T) {
	...
	svc := NewKeepAliveListenKeyService("API_KEY", "SECRET_KEY").
...
func TestKeepAliveListenKeyService_Do_Errors(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			svc := NewKeepAliveListenKeyService("", "").WithClient(client)
			ctx := context.Background()
			_, err := svc.Do(ctx)
