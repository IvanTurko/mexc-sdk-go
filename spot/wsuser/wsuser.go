...
// Subscription describes a WebSocket subscription (orders, deals, etc.).
// Instances are normally created via constructors like NewSpotAccountOrdersSub().
...
// WSUser is a WebSocket client for MEXC SPOT user streams.
type WSUser struct {
	client         ws.Client
...
// Panics if listenKey is empty.
func NewWSUser(key string, opts ...Options) *WSUser {
	if key == "" {
		panic("NewWSUser: listenKey is required")
	}

	factory := func(url string) ws.Client {
		return ws.NewClient(url)
	}
	return NewWSUserWithFactory(key, factory, opts...)
}
...
// Panics if listenKey is empty or factory is nil.
func NewWSUserWithFactory(key string, factory func(url string) ws.Client, opts ...Options) *WSUser {
	if key == "" {
		panic("NewWSUserWithFactory: listenKey is required")
	}
	...
	url := fmt.Sprintf("%s?listenKey=%s", defaultBaseURL, key)
...
//   - ErrMaxCountSubscribes: subscription limit exceeded (max 3 for user streams).
//
...
func (w *WSUser) handleJSONMessage(data []byte) {
	var msg *message
...
func (w *WSUser) handleProtoMessage(data []byte) {
	var msg PushDataV3UserWrapper
